defmodule Amarula.Protocol.Binary.Constants do
  @moduledoc """
  WhatsApp binary protocol constants.
  Ported from src/WABinary/constants.ts
  """
  ...
  # Single byte tokens (from Baileys SINGLE_BYTE_TOKENS)
  @single_byte_tokens [
    "",
    "xmlstreamstart",
    "xmlstreamend",
  ...
  # Double byte tokens - all four dictionaries (from Baileys)
  # These are compiled into @double_byte_token_map below
  @double_byte_tokens [
    # Dictionary 0
    [
      "read-self",
  ...
  @tag_map %{
    ...
    dictionary_0: @dictionary_0,
    dictionary_1: @dictionary_1,
    dictionary_2: @dictionary_2,
    dictionary_3: @dictionary_3,
    interop_jid: @interop_jid,
    fb_jid: @fb_jid,
    ad_jid: @ad_jid,
    list_8: @list_8,
    list_16: @list_16,
  ...
  # Reverse tag lookup (commented out as not used yet)
  # @reverse_tag_map Enum.into(@tag_map, %{}, fn {k, v} -> {v, k} end)

  # Token lookup maps
  @single_byte_token_map Enum.with_index(@single_byte_tokens) |> Enum.into(%{})
  @reverse_single_byte_map Enum.into(@single_byte_token_map, %{}, fn {k, v} -> {v, k} end)

  # Build double-byte token map from all four dictionaries
  @double_byte_token_map @double_byte_tokens
                         |> Enum.with_index()
                         |> Enum.flat_map(fn {tokens, dict_index} ->
                           tokens
                           |> Enum.with_index()
                           |> Enum.map(fn {token, token_index} ->
                             {token, {dict_index, token_index}}
                           end)
                         end)
                         |> Enum.into(%{})

  @reverse_double_byte_map Enum.into(@double_byte_token_map, %{}, fn {k, v} -> {v, k} end)

  @doc "Get tag value by atom"
  @spec tag(atom()) :: integer()
  def tag(tag_atom) do
    Map.get(@tag_map, tag_atom)
  end

  @doc "Convert string to tag (single or double byte)"
  @spec string_to_tag(binary()) :: integer() | {integer(), integer()} | nil
  def string_to_tag(string) do
    cond do
      Map.has_key?(@single_byte_token_map, string) ->
        Map.get(@single_byte_token_map, string)

      Map.has_key?(@double_byte_token_map, string) ->
        Map.get(@double_byte_token_map, string)
  ...
  @doc "Convert tag to string"
  @spec tag_to_string(integer() | {integer(), integer()}) :: binary() | nil
  def tag_to_string(tag) when is_integer(tag) do
    Map.get(@reverse_single_byte_map, tag)
  end

  def tag_to_string({dict, index}) do
    Map.get(@reverse_double_byte_map, {dict, index})
  end

  @doc "Convert token index to string"
  @spec token_to_string(integer()) :: binary() | nil
  def token_to_string(index) when is_integer(index) and index >= 0 do
    Map.get(@reverse_single_byte_map, index)
  end

  def token_to_string(_), do: nil

  @doc "Convert string to token index"
  @spec string_to_token(binary()) :: integer() | nil
  def string_to_token(string) do
    Map.get(@single_byte_token_map, string)
  end
