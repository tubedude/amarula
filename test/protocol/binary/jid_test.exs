defmodule Amarula.Protocol.Binary.JIDTest do
  ...
  describe "predicates" do
    test "is_jid_user?/1 identifies user JIDs" do
      assert JID.is_jid_user?("1234@s.whatsapp.net") == true
      assert JID.is_jid_user?("1234@g.us") == false
      assert JID.is_jid_user?("1234@lid") == true
      assert JID.is_jid_user?("1234@hosted") == true
    end

    test "is_jid_group?/1 identifies group JIDs" do
      assert JID.is_jid_group?("1234@g.us") == true
      assert JID.is_jid_group?("1234@s.whatsapp.net") == false
    end

    test "is_lid_user?/1 identifies LID users" do
      assert JID.is_lid_user?("1234@lid") == true
      assert JID.is_lid_user?("1234@s.whatsapp.net") == false
    end

    test "is_jid_broadcast?/1 identifies broadcast JIDs" do
      assert JID.is_jid_broadcast?("1234@broadcast") == true
      assert JID.is_jid_broadcast?("1234@s.whatsapp.net") == false
    end

    test "is_jid_bot?/1 identifies bots" do
      # From TypeScript: /^1313555\d{4}$|^131655500\d{2}$/
      assert JID.is_jid_bot?("13135551234@c.us") == true
      assert JID.is_jid_bot?("13165550012@c.us") == true
      assert JID.is_jid_bot?("1234@c.us") == false
      assert JID.is_jid_bot?("13135551234@s.whatsapp.net") == false
    end

    test "is_jid_newsletter?/1 identifies newsletter JIDs" do
      assert JID.is_jid_newsletter?("1234@newsletter") == true
      assert JID.is_jid_newsletter?("1234@s.whatsapp.net") == false
    end

    test "is_hosted_pn_user?/1 identifies hosted PN users" do
      assert JID.is_hosted_pn_user?("1234@hosted") == true
      assert JID.is_hosted_pn_user?("1234@s.whatsapp.net") == false
    end

    test "is_hosted_lid_user?/1 identifies hosted LID users" do
      assert JID.is_hosted_lid_user?("1234@hosted.lid") == true
      assert JID.is_hosted_lid_user?("1234@lid") == false
    end

    test "is_jid_status_broadcast?/1 identifies status broadcast" do
      assert JID.is_jid_status_broadcast?("status@broadcast") == true
      assert JID.is_jid_status_broadcast?("1234@broadcast") == false
    end

    test "is_jid_meta_ai?/1 identifies Meta AI" do
      assert JID.is_jid_meta_ai?("1234@bot") == true
      assert JID.is_jid_meta_ai?("1234@s.whatsapp.net") == false
    end
  end
  ...
  describe "group?/1 alias" do
    test "group?/1 is alias for is_jid_group?/1" do
      assert JID.group?("1234@g.us") == JID.is_jid_group?("1234@g.us")
      assert JID.group?("1234@s.whatsapp.net") == JID.is_jid_group?("1234@s.whatsapp.net")
      assert JID.group?(nil) == JID.is_jid_group?(nil)
    end
  end

  describe "predicate edge cases" do
    test "predicates handle nil gracefully" do
      assert JID.is_jid_user?(nil) == false
      assert JID.is_jid_group?(nil) == false
      assert JID.is_lid_user?(nil) == false
      assert JID.is_jid_broadcast?(nil) == false
      assert JID.is_jid_bot?(nil) == false
      assert JID.is_jid_newsletter?(nil) == false
      assert JID.is_hosted_pn_user?(nil) == false
      assert JID.is_hosted_lid_user?(nil) == false
      assert JID.is_jid_meta_ai?(nil) == false
    end

    test "predicates handle non-binary input" do
      assert JID.is_jid_user?(123) == false
      assert JID.is_jid_group?(%{}) == false
      assert JID.is_lid_user?([]) == false
    end
  end
